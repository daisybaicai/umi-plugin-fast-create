...
import defaultInfoContentTemplate  from '../../templates/default/InfoContent';
...
async function handleComponents(absoultPath, jsonData) {
  ...
  const stateName =  createStateName(urlTransform(jsonData.api.url), 'Detail');
...
const handleDetail =  async (api, text) => {
  ...
  if (r.valid) {
    ...
    // 3. 创建依赖的dep
    handleDepend(absoultPath, jsonData);
    // 4. 创建components
